...
import { getIdFromNodeObjectIfDefined, type ReferenceNodeObject } from 'rmlmapper-js';
...
import { DOGM, SHACL, RDFS, DOGM_ENGINE, XSD, RDF } from './util/Vocabularies';
...
export class DOGMEngine {
  ...
  private async findMappingForVerbContextually(
    ...
    if (args.account) {
      ...
      const integrationId = (account[DOGM.integration] as ReferenceNodeObject)['@id'];
  ...
  private async executeParallelMapping(
    ...
    const parallelVerbMappings = ensureArray(mapping[DOGM.parallel] as unknown as OrArray<VerbMapping>);
  ...
  private async performOperation(
    ...
    if (operationInfo[DOGM.dataSource]) {
      return await this.getDataFromDataSource(
        getIdFromNodeObjectIfDefined(operationInfo[DOGM.dataSource] as string | ReferenceNodeObject)!,
        verbConfig,
      );
    }
  ...
  private async performParameterMappingOnArgsIfDefined(
    ...
    if (DOGM.parameterReference in mapping) {
      const reference = getValueIfDefined<string>(mapping[DOGM.parameterReference])!;
  ...
  private async getOpenApiDescriptionForIntegration(integrationId: string): Promise<OpenApi> {
    ...
    return getValueIfDefined<OpenApi>(openApiDescriptionSchema[DOGM.openApiDescription])!;
  }
  ...
  private async assertVerbParamsMatchParameterSchemas(verbParams: any, verb: Verb): Promise<void> {
    ...
    if (verbParams && parametersSchemaObject) {
      const verbParamsAsJsonLd = {
        '@context': getValueIfDefined<ContextDefinition>(verb[DOGM.parametersContext]),
  ...
  private async performOpenapiOperationWithCredentials(
    ...
    const configuration = {
      accessToken: getValueIfDefined<string>(securityCredentials?.[DOGM.accessToken]),
      bearerToken: getValueIfDefined<string>(securityCredentials?.[DOGM.bearerToken]),
  ...
  private async isInvalidTokenError(error: AxiosError, integrationId: string): Promise<boolean> {
    ...
    const errorMatcher = integration[DOGM.invalidTokenErrorMatcher] as NodeObject;
    const errorMatcherStatus = errorMatcher &&
      getValueIfDefined<string>(errorMatcher[DOGM.invalidTokenErrorMatcherStatus]);
    const errorMatcherRegex = errorMatcher &&
      getValueIfDefined<string>(errorMatcher[DOGM.invalidTokenErrorMatcherMessageRegex])!;
  ...
  private async refreshSecurityCredentials(
    ...
    const getOauthTokenVerb = (await this.findBy({ type: DOGM.Verb, [RDFS.label]: 'getOauthTokens' })) as Verb;
    ...
    const args = {
      refreshToken: getValueIfDefined<string>(securityCredentials[DOGM.refreshToken])!,
      jwtBearerOptions: getValueIfDefined<string>(securityCredentials[DOGM.jwtBearerOptions])!,
    };
    ...
    const bearerToken = getValueIfDefined<string>(mappedReturnValue[DOGM.bearerToken]);
    const accessToken = getValueIfDefined<string>(mappedReturnValue[DOGM.accessToken]);
    const refreshToken = getValueIfDefined<string>(mappedReturnValue[DOGM.refreshToken]);
    if (bearerToken) {
  ...
  private getOauthConfigurationFromSecurityCredentials(
    ...
    const username = getValueIfDefined<string>(securityCredentialsSchema[DOGM.clientId]);
    const password = getValueIfDefined<string>(securityCredentialsSchema[DOGM.clientSecret]);
    const accessToken = getValueIfDefined<string>(securityCredentialsSchema[DOGM.accessToken]);
  ...
  private async performOauthSecuritySchemeStageWithCredentials(
    ...
    if (securityCredentials) {
      ...
      operationParameters.client_id = getValueIfDefined<string>(securityCredentials[DOGM.clientId])!;
    } else {
      configuration = {};
    }
    const openApiExecutor = await this.createOpenApiOperationExecutorWithSpec(openApiDescription);
