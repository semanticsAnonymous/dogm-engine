...
import { SDO, DOGM, OWL, RDFS, DCTERMS } from '../../src/util/Vocabularies';
...
describeIf('docker', 'An DOGM engine backed by a sparql query adapter', (): void => {
  let engine: DOGMEngine;
  let schema: Entity[];
  ...
  beforeAll(async(): Promise<void> => {
    ...
    engine = new DOGMEngine({ type: 'sparql', endpointUrl, setTimestamps: true });
    ...
    const nouns = await engine.findAll({
      where: {
        type: In([
          ...
          'https://dogm.com/ontologies/core/Account',
        ]),
      },
      order: {
        [RDFS.label]: 'asc',
