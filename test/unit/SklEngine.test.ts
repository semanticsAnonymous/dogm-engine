...
import { RR } from 'rmlmapper-js';
import { DOGMEngine } from '../../src/DogmEngine';
import type { DogmEngineOptions } from '../../src/DogmEngineOptions';
...
const expectedGetFileResponse = {
  ...
  '@type': 'https://dogm.com/ontologies/core/File',
  'https://dogm.com/ontologies/core/deleted': {
    '@type': 'http://www.w3.org/2001/XMLSchema#boolean',
    '@value': false,
  },
  'https://dogm.com/ontologies/core/integration': {
    '@id': 'https://example.com/integrations/Dropbox',
  },
  'https://dogm.com/ontologies/core/isWeblink': {
    '@type': 'http://www.w3.org/2001/XMLSchema#boolean',
    '@value': false,
  },
  'https://dogm.com/ontologies/core/mimeType': 'text/plain',
  'http://www.w3.org/2000/01/rdf-schema#label': 'Prime_Numbers.txt',
  'https://dogm.com/ontologies/core/size': {
    '@type': 'http://www.w3.org/2001/XMLSchema#integer',
    '@value': 7212,
  },
  'https://dogm.com/ontologies/core/sourceId': 'id:12345',
};
...
const incorrectReturnValueMapping = {
  ...
  'http://www.w3.org/ns/r2rml#predicateObjectMap': [
    {
      ...
      'http://www.w3.org/ns/r2rml#predicate': { '@id': 'https://dogm.com/ontologies/core/Integration' },
    },
  ],
  'http://www.w3.org/ns/r2rml#subjectMap': {
    ...
    'http://www.w3.org/ns/r2rml#class': { '@id': 'https://dogm.com/ontologies/core/File' },
  },
};

describe('DOGMEngine', (): void => {
  let schemas: any[];
  ...
  describe('Memory', (): void => {
    it('initializes.', async(): Promise<void> => {
      expect(new DOGMEngine({ type: 'memory' })).toBeInstanceOf(DOGMEngine);
    });
  });
  ...
  describe('Sparql', (): void => {
    it('initializes.', async(): Promise<void> => {
      ...
      expect(new DOGMEngine({ type: 'sparql', endpointUrl: sparqlEndpoint })).toBeInstanceOf(DOGMEngine);
    });
  });

  describe('CRUD on schemas', (): void => {
    let dogmEngine: DOGMEngine;

    beforeEach(async(): Promise<void> => {
      schemas = [{
        '@id': 'https://dogm.com/ontologies/core/Share',
        '@type': 'https://dogm.com/ontologies/core/Verb',
      }];
      dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
    });
    ...
    describe('executeRawQuery', (): void => {
      it('delegates calls to executeRawQuery to the query adapter.', async(): Promise<void> => {
        ...
        await expect(dogmEngine.executeRawQuery('')).resolves.toEqual([]);
    ...
    describe('executeRawUpdate', (): void => {
      it('delegates calls to executeRawUpdate to the query adapter.', async(): Promise<void> => {
        ...
        await expect(
          dogmEngine.executeRawUpdate(`DELETE { ?s ?p ?o } WHERE { ?s ?p ?o }`),
    ...
    describe('executeRawConstructQuery', (): void => {
      it('delegates calls to executeRawConstructQuery to the query adapter.', async(): Promise<void> => {
        ...
        await expect(dogmEngine.executeRawConstructQuery(
    ...
    describe('find', (): void => {
      it('delegates calls to find to the query adapter.', async(): Promise<void> => {
        ...
        await expect(dogmEngine.find({ where: { id: 'https://dogm.com/ontologies/core/Share' }})).resolves.toEqual(schemas[0]);
        expect(findSpy).toHaveBeenCalledTimes(1);
        expect(findSpy).toHaveBeenCalledWith({ where: { id: 'https://dogm.com/ontologies/core/Share' }});
      });

      it('throws an error if there is no schema matching the query during find.', async(): Promise<void> => {
        const findSpy = jest.spyOn(SparqlQueryAdapter.prototype, 'find');
        await expect(dogmEngine.find({ where: { id: 'https://dogm.com/ontologies/core/Send' }})).rejects.toThrow(
          'No schema found with fields matching {"where":{"id":"https://dogm.com/ontologies/core/Send"}}',
        );
        expect(findSpy).toHaveBeenCalledTimes(1);
        expect(findSpy).toHaveBeenCalledWith({ where: { id: 'https://dogm.com/ontologies/core/Send' }});
      });
    });
    ...
    describe('findBy', (): void => {
      it('delegates calls to findBy to the query adapter.', async(): Promise<void> => {
        ...
        await expect(dogmEngine.findBy({ id: 'https://dogm.com/ontologies/core/Share' })).resolves.toEqual(schemas[0]);
        expect(findBySpy).toHaveBeenCalledTimes(1);
        expect(findBySpy).toHaveBeenCalledWith({ id: 'https://dogm.com/ontologies/core/Share' });
      });

      it('throws an error if there is no schema matching the query during findBy.', async(): Promise<void> => {
        const findBySpy = jest.spyOn(SparqlQueryAdapter.prototype, 'findBy');
        await expect(dogmEngine.findBy({ id: 'https://dogm.com/ontologies/core/Send' })).rejects.toThrow(
          'No schema found with fields matching {"id":"https://dogm.com/ontologies/core/Send"}',
        );
        expect(findBySpy).toHaveBeenCalledTimes(1);
        expect(findBySpy).toHaveBeenCalledWith({ id: 'https://dogm.com/ontologies/core/Send' });
      });
    });
    ...
    describe('findAll', (): void => {
      it('delegates calls to findAll to the query adapter.', async(): Promise<void> => {
        ...
        await expect(dogmEngine.findAll({ where: { id: 'https://dogm.com/ontologies/core/Share' }})).resolves.toEqual([ schemas[0] ]);
        expect(findAllSpy).toHaveBeenCalledTimes(1);
        expect(findAllSpy).toHaveBeenCalledWith({ where: { id: 'https://dogm.com/ontologies/core/Share' }});
      });
    });
    ...
    describe('findAllBy', (): void => {
      it('delegates calls to findAllBy to the query adapter.', async(): Promise<void> => {
        ...
        await expect(dogmEngine.findAllBy({ id: 'https://dogm.com/ontologies/core/Share' })).resolves.toEqual([ schemas[0] ]);
        expect(findAllBySpy).toHaveBeenCalledTimes(1);
        expect(findAllBySpy).toHaveBeenCalledWith({ id: 'https://dogm.com/ontologies/core/Share' });
      });
    });
    ...
    describe('exists', (): void => {
      it('delegates calls to exists to the query adapter.', async(): Promise<void> => {
        ...
        await expect(dogmEngine.exists({ where: { id: 'https://dogm.com/ontologies/core/Share' }})).resolves.toBe(true);
        expect(existsSpy).toHaveBeenCalledTimes(1);
        expect(existsSpy).toHaveBeenCalledWith({ where: { id: 'https://dogm.com/ontologies/core/Share' }});
      });
    });
    ...
    describe('count', (): void => {
      it('delegates calls to count to the query adapter.', async(): Promise<void> => {
        ...
        await expect(dogmEngine.count({ where: { id: 'https://dogm.com/ontologies/core/Share' }})).resolves.toBe(1);
        expect(countSpy).toHaveBeenCalledTimes(1);
        expect(countSpy).toHaveBeenCalledWith({ where: { id: 'https://dogm.com/ontologies/core/Share' }});
      });
    });
    ...
    describe('save', (): void => {
      ...
      it('delegates calls to save a single entity to the query adapter.', async(): Promise<void> => {
        ...
        const res = await dogmEngine.save({
          '@id': 'https://dogm.com/ontologies/core/Share',
          '@type': 'https://dogm.com/ontologies/core/Verb',
          [RDFS.label]: 'Share',
        });
      ...
      it('delegates calls to save multiple entities to the query adapter.', async(): Promise<void> => {
        ...
        const entities = [
          ...
          {
            '@id': 'https://dogm.com/ontologies/core/Send',
            '@type': 'https://dogm.com/ontologies/core/Verb',
            [RDFS.label]: 'Send',
          },
        ];
        const res = await dogmEngine.save(entities);
      ...
      it('throws an error if the entity does not conform to the schema.', async(): Promise<void> => {
        ...
        await expect(dogmEngine.save(entity)).rejects.toThrow(
          'Entity https://example.com/data/1 does not conform to the https://dogm.com/ontologies/core/File schema',
        );
      ...
      it('throws an error if the entity does not conform to the parent schema.', async(): Promise<void> => {
        ...
        const entity = {
          '@id': 'https://example.com/data/1',
          '@type': 'https://dogm.com/ontologies/core/Folder',
        };
        await expect(dogmEngine.save(entity)).rejects.toThrow(
          'Entity https://example.com/data/1 does not conform to the https://dogm.com/ontologies/core/File schema',
        );
      ...
      it('throws an error if one of the entities does not conform to the schema.', async(): Promise<void> => {
        ...
        await expect(dogmEngine.save(entities)).rejects.toThrow(
          'An entity does not conform to the https://dogm.com/ontologies/core/File schema',
        );
        expect(saveSpy).toHaveBeenCalledTimes(0);
      });
    ...
    describe('update', (): void => {
      ...
      beforeEach(async(): Promise<void> => {
        schemas = await frameAndCombineSchemas([ './test/assets/schemas/core.json' ]);
        await dogmEngine.save(schemas);
        await dogmEngine.save(entities);
      });
      ...
      it('delegates calls to update a single entity to the query adapter.', async(): Promise<void> => {
        ...
        const res = await dogmEngine.update(
          entities[0]['@id'],
          { [RDFS.label]: 'file1' },
        );
      ...
      it('throws an error if the entity does not conform to the schema.', async(): Promise<void> => {
        const updateSpy = jest.spyOn(SparqlQueryAdapter.prototype, 'update');
        await expect(dogmEngine.update(entities[0]['@id'], badAttributes)).rejects.toThrow(
          'Entity https://example.com/data/1 does not conform to the https://dogm.com/ontologies/core/File schema',
        );
      ...
      it('throws an error if the entity does not conform to the parent schema.', async(): Promise<void> => {
        ...
        await dogmEngine.save(entity);
        await expect(dogmEngine.update(entity['@id'], badAttributes)).rejects.toThrow(
          'Entity https://example.com/data/1 does not conform to the https://dogm.com/ontologies/core/File schema',
        );
        expect(updateSpy).toHaveBeenCalledTimes(0);
      });
      ...
      it('throws an error if one of the entities does not conform to the schema.', async(): Promise<void> => {
        const updateSpy = jest.spyOn(SparqlQueryAdapter.prototype, 'update');
        await expect(dogmEngine.update(entityIds, badAttributes)).rejects.toThrow(
          'Entity https://example.com/data/1 does not conform to the https://dogm.com/ontologies/core/File schema',
        );
        expect(updateSpy).toHaveBeenCalledTimes(0);
      });
    ...
    describe('delete', (): void => {
      it('delegates calls to delete a single entity to the query adapter.', async(): Promise<void> => {
        ...
        await dogmEngine.save(entity);
        const res = await dogmEngine.delete(entity['@id']);
        expect(res).toBeUndefined();
      ...
      it('delegates calls to delete miltiple entities to the query adapter.', async(): Promise<void> => {
        ...
        const entityIds = entities.map((entity): string => entity['@id']);
        await dogmEngine.save(entities);
        const res = await dogmEngine.delete(entityIds);
        expect(res).toBeUndefined();
        expect(deleteSpy).toHaveBeenCalledTimes(1);
    ...
    describe('destroy', (): void => {
      it('delegates calls to destroy a single entity to the query adapter.', async(): Promise<void> => {
        ...
        await dogmEngine.save(entity);
        const res = await dogmEngine.destroy(entity);
      ...
      it('delegates calls to destroy miltiple entities to the query adapter.', async(): Promise<void> => {
        ...
        await dogmEngine.save(entities);
        const res = await dogmEngine.destroy(entities);
        expect(res).toEqual(entities);
        expect(destroySpy).toHaveBeenCalledTimes(1);
    ...
    describe('destroyAll', (): void => {
      it('delegates calls to destroyAll to the query adapter.', async(): Promise<void> => {
        ...
        const res = await dogmEngine.destroyAll();
        expect(res).toBeUndefined();
  ...
  describe('mapping data', (): void => {
    ...
    it('maps data with a frame.', async(): Promise<void> => {
      ...
      const response = await dogmEngine.performMapping(data, mapping as NodeObject, frame);
  ...
  describe('calling Verbs which execute OpenApi operations', (): void => {
    ...
    it('can execute an OpenApi operation defined via an operationMapping.', async(): Promise<void> => {
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      const response = await dogmEngine.verb.getFile({ account, id: '12345' });
    ...
    it('can execute an OpenApi operation defined via a constant operationId.', async(): Promise<void> => {
      schemas = schemas.map((schemaItem: any): any => {
        if (schemaItem['@id'] === 'https://example.com/data/4') {
          schemaItem[DOGM.operationId] = {
            '@type': XSD.string,
            '@value': 'FilesGetMetadata',
          };
          // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
          delete schemaItem[DOGM.operationMapping];
        }
        return schemaItem;
      });
      const dogmEngine = new DOGMEngine({ type: 'memory' });
    ...
    it('returns the raw operation response if the verb integration mapping does not have a return value mapping.',
      async(): Promise<void> => {
        schemas = schemas.map((schemaItem: any): any => {
          if (schemaItem['@id'] === 'https://example.com/data/4') {
            // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
            delete schemaItem[DOGM.returnValueMapping];
          }
          return schemaItem;
        });
        const dogmEngine = new DOGMEngine({ type: 'memory' });
    ...
    it('errors if the executed verb is not defined.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      await expect(dogmEngine.verb.getFile({ account, id: '12345' })).rejects.toThrow(
    ...
    it('errors if the parameters do not conform to the verb parameter schema.', async(): Promise<void> => {
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      await expect(dogmEngine.verb.getFile({ account, ids: [ '12345' ]})).rejects.toThrow(
    ...
    it('validates using the return value\'s type if it does not have an id.', async(): Promise<void> => {
      schemas.forEach((schemaItem: any): void => {
        if (schemaItem['@id'] === 'https://example.com/data/4') {
          // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
          delete schemaItem[DOGM.returnValueMapping][RR.subject];
          schemaItem[DOGM.returnValueMapping][RR.subjectMap] = {
            '@type': 'rr:SubjectMap',
            [RR.termType]: { '@id': RR.BlankNode },
            [RR.class]: { '@id': 'https://dogm.com/ontologies/core/File' },
          };
        }
      });
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      ...
      await expect(dogmEngine.verb.getFile({ account, id: '12345' })).resolves.toEqual(getFileResponseWithoutId);
      expect(executeOperation).toHaveBeenCalledTimes(1);
      expect(executeOperation).toHaveBeenCalledWith(
        'FilesGetMetadata',
        { accessToken: 'SPOOFED_TOKEN', bearerToken: undefined, apiKey: undefined, basePath: undefined },
    ...
    it('errors if the return value does not conform to the verb return value schema.', async(): Promise<void> => {
      schemas.forEach((schemaItem: any): void => {
        if (schemaItem['@id'] === 'https://example.com/data/4') {
          schemaItem[DOGM.returnValueMapping] = incorrectReturnValueMapping;
        }
      });
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
    ...
    it('errors if no open api description for the integration is in the schema.', async(): Promise<void> => {
      ...
      await expect(dogmEngine.verb.getFile({ account, id: '12345' })).rejects.toThrow([
        ...
        '{"type":"https://dogm.com/ontologies/core/OpenApiDescription","https://dogm.com/ontologies/core/integration":"https://example.com/integrations/Dropbox"}',
      ].join(' '));
      expect(executeOperation).toHaveBeenCalledTimes(0);
    });
    ...
    async(): Promise<void> => {
      ...
      expect(executeSecuritySchemeStage).toHaveBeenCalledWith(
        ...
        'authorizationCode',
        'tokenUrl',
        { username: 'semanticsAnonymous', password: 'abc123', accessToken: 'SPOOFED_TOKEN' },
        { client_id: 'semanticsAnonymous', grant_type: 'refresh_token', refresh_token: 'SPOOFED_REFRESH_TOKEN' },
      );
    });
    ...
    async(): Promise<void> => {
      schemas = schemas.map((schemaItem: any): any => {
        if (schemaItem['@id'] === 'https://example.com/integrations/Dropbox') {
          schemaItem[DOGM.invalidTokenErrorMatcher] = {
            '@type': DOGM.InvalidTokenErrorMatcher,
            [DOGM.invalidTokenErrorMatcherStatus]: 400,
          };
        }
        return schemaItem;
      });
    ...
    it('throws an error if the operation fails with an error other than invalid token.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      await expect(dogmEngine.verb.getFile({ account, id: '12345' })).rejects.toThrow('Internal Server Error');
      expect(executeOperation).toHaveBeenCalledTimes(1);
      expect(executeOperation).toHaveBeenNthCalledWith(
        1,
        'FilesGetMetadata',
  ...
  describe('calling Verbs which execute OpenApi security schemes', (): void => {
    ...
    it('can execute an OpenApiSecuritySchemeVerb that maps to the authorizationUrl stage.', async(): Promise<void> => {
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      await expect(dogmEngine.verb.authorizeWithPkceOauth({ account })).resolves.toEqual({
        data: response,
        operationParameters: {
          account,
          client_id: 'semanticsAnonymous',
        },
      });
      expect(executeSecuritySchemeStage).toHaveBeenCalledTimes(1);
      expect(executeSecuritySchemeStage).toHaveBeenCalledWith(
        ...
        'authorizationUrl',
        { username: 'semanticsAnonymous', password: 'abc123', accessToken: 'SPOOFED_TOKEN' },
        { client_id: 'semanticsAnonymous', account },
      );
    });
    ...
    it('errors if the executed verb is not defined.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      await expect(dogmEngine.verb.getFile({ account, id: '12345' })).rejects.toThrow(Error);
      await expect(dogmEngine.verb.getFile({ account, id: '12345' })).rejects.toThrow(
        'Failed to find the verb getFile in the schema',
      );
    });
    ...
    it('can execute an OpenApiSecuritySchemeVerb that maps to the tokenUrl stage.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      const res = await dogmEngine.verb.getOauthTokens<NodeObject>({
        account,
        codeVerifier: 'something',
        code: 'dummy_code',
      });
      expect(res[DOGM.accessToken]).toBe('abc123');
      expect(executeSecuritySchemeStage).toHaveBeenCalledTimes(1);
      expect(executeSecuritySchemeStage).toHaveBeenCalledWith(
        'oAuth',
        'authorizationCode',
    ...
    async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      const res = await dogmEngine.verb.authorizeWithPkceOauth<NodeObject>({ account });
      expect(res[DOGM.accessToken]).toBeUndefined();
      expect(executeSecuritySchemeStage).toHaveBeenCalledTimes(1);
      expect(executeSecuritySchemeStage).toHaveBeenCalledWith(
        'oAuth',
        'authorizationCode',
    ...
    it('can execute an OpenApiSecuritySchemeVerb that maps to the tokenUrl stage with credentials.',
      async(): Promise<void> => {
        ...
        const dogmEngine = new DOGMEngine({ type: 'memory' });
        await dogmEngine.save(schemas);
        const res = await dogmEngine.verb.getOauthTokens<NodeObject>({ account: 'https://example.com/data/StubhubAccount1' });
        expect(res[DOGM.accessToken]).toBe('abc123');
        expect(executeSecuritySchemeStage).toHaveBeenCalledTimes(1);
        expect(executeSecuritySchemeStage).toHaveBeenCalledWith(
          'OAuth2',
          ...
          { grant_type: 'client_credentials', scope: 'read:events', client_id: 'semanticsAnonymous' },
        );
      });
  });
  ...
  describe('calling Verbs with a specific mapping', (): void => {
    it('can execute the verb with the mapping.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory', functions });
      await dogmEngine.save(schemas);
      const entity = {
        '@id': 'https://example.com/data/1',
      ...
      const response = await dogmEngine.verb.parseAndSaveLinksFromEntity({
        entity,
  ...
  describe('calling Verbs which map a Noun to another Verb', (): void => {
    ...
    it('can execute a Noun mapped Verb defined via a verbMapping.', async(): Promise<void> => {
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      const response = await dogmEngine.verb.sync({
        noun: 'https://dogm.com/ontologies/core/File',
        account,
        id: '12345',
      });
      expect(response).toEqual(expectedGetFileResponse);
    ...
    it('can execute a Noun mapped Verb with only a mapping.', async(): Promise<void> => {
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      const response = await dogmEngine.verb.getName({
        noun: 'https://dogm.com/ontologies/core/File',
        entity: { [RDFS.label]: 'final.jpg', [DOGM.sourceId]: 12345 },
      });
      expect(response).toEqual({
      ...
      async(): Promise<void> => {
        schemas = schemas.map((schemaItem: any): any => {
          if (schemaItem['@id'] === 'https://example.com/data/34') {
            schemaItem[DOGM.verbId] = {
              '@type': XSD.string,
              '@value': 'https://example.com/getFile',
            };
            // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
            delete schemaItem[DOGM.verbMapping];
          }
          return schemaItem;
        });
        const dogmEngine = new DOGMEngine({ type: 'memory' });
  ...
  describe('calling Verbs which use data from a data source', (): void => {
    it('gets data from a JsonDataSource from the data field.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      const response = await dogmEngine.verb.getFile({
        account: 'https://example.com/data/JsonSourceAccount1',
        id: '12345',
      });
      expect(response).toEqual({
        ...expectedGetFileResponse,
        [DOGM.integration]: { '@id': 'https://example.com/integrations/JsonSource' },
      });
    });
    ...
    it('gets data from a JsonDataSource from the source field.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({
        type: 'memory',
        inputFiles: {
          'data.json': `{
            ".tag": "file",
    ...
    it('throws an error if a json data source was not provided.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory', inputFiles: {}});
      await dogmEngine.save(schemas);
      await expect(dogmEngine.verb.getFile({
        account: 'https://example.com/data/JsonSourceAccount1',
        id: '12345',
      }))
    ...
    it('throws an error for an invalid DataSource.', async(): Promise<void> => {
      ...
      schemas = schemas.map((schemaItem: any): any => {
        if (schemaItem['@id'] === 'https://example.com/data/JsonFileDataSource') {
          schemaItem['@type'] = 'https://dogm.com/ontologies/core/CsvDataSource';
        }
        return schemaItem;
      });
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      ...
      await expect(dogmEngine.verb.getFile({
        ...
        .rejects.toThrow('DataSource type https://dogm.com/ontologies/core/CsvDataSource is not supported.');
    });
  });
  ...
  describe('calling Verbs which specify series composite mapping', (): void => {
    it('can execute multiple Verbs in series.', async(): Promise<void> => {
      ...
      const response = await dogmEngine.verb.parseAndSaveLinksFromEntity({ entity });
      expect(response).toEqual({});
    });
    ...
    it('does not run a verb from a series mapping if its verbMapping does not return a verbId.',
      async(): Promise<void> => {
        ...
        const dogmEngine = new DOGMEngine({ type: 'memory' });
        await dogmEngine.save(schemas);
        const response = await dogmEngine.verb.transformText({ text: 'Hello' });
        expect(response).toEqual(
          expect.objectContaining({
            text: 'Hello',
          }),
  ...
  describe('calling Verbs which use a parallel composite mapping', (): void => {
    ...
    it('can execute multiple Verbs in parallel.', async(): Promise<void> => {
      ...
      const response = await dogmEngine.verb.parseLinksAndCountCharactersFromEntity({ entity });
      expect(response).toEqual([
        {
          '@context': {
            links: {
    ...
    it('can execute multiple Verbs in with return values that have ids.', async(): Promise<void> => {
      ...
      schemas = schemas.map((schemaItem: any): any => {
        if (schemaItem['@id'] === 'https://example.com/parseLinksAndCountCharactersFromEntityMapping') {
          // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
          delete schemaItem[DOGM.parallel][0][DOGM.returnValueMapping][RR.subjectMap][RR.class];
          schemaItem[DOGM.parallel][0][DOGM.returnValueMapping][RR.subjectMap][RR.constant] = 'https://example.com/res/1';
          // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
          delete schemaItem[DOGM.parallel][1][DOGM.returnValueMapping][RR.subjectMap][RR.class];
          schemaItem[DOGM.parallel][1][DOGM.returnValueMapping][RR.subjectMap][RR.constant] = 'https://example.com/res/2';
        }
        return schemaItem;
      });
      const dogmEngine = new DOGMEngine({ type: 'memory', functions });
  ...
  describe('calling Verbs which execute operations on entities', (): void => {
    it('counts entities.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      const response = await dogmEngine.verb.countEntities({ where: { type: DOGM.Verb }});
      expect(response).toEqual(
        expect.objectContaining({
    ...
    it('finds if entities exist.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      const response = await dogmEngine.verb.entitiesExist({ where: { type: DOGM.Verb }});
      expect(response).toEqual(
        expect.objectContaining({
    ...
    it('saves entities.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      const response = await dogmEngine.verb.saveEntities({ entities });
    ...
    it('destroy entities.', async(): Promise<void> => {
      ...
      await dogmEngine.save(schemas);
      await dogmEngine.save(entities);
      const response = await dogmEngine.verb.destroyEntities({ entities });
      expect(response).toEqual(entities[0]);
    });
    ...
    it('finds an entity.', async(): Promise<void> => {
      ...
      await dogmEngine.save(schemas);
      await dogmEngine.save(entity);
      const response = await dogmEngine.verb.findEntity({ where: { id: entity['@id'] }});
    ...
    it('finds multiple entities.', async(): Promise<void> => {
      ...
      await dogmEngine.save(schemas);
      await dogmEngine.save(entities);
      const response = await dogmEngine.verb.findAllEntities({ where: { type: 'https://schema.org/BlogPosting' }});
  ...
  describe('calling Triggers', (): void => {
    ...
    it('can execute a Verb as result of a trigger.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      await dogmEngine.executeTrigger(
    ...
    it('throws an error when no Trigger Verb Mapping exists for the integration.', async(): Promise<void> => {
      ...
      const dogmEngine = new DOGMEngine({ type: 'memory' });
      await dogmEngine.save(schemas);
      await expect(dogmEngine.executeTrigger(
  ...
  describe('callbacks', (): void => {
    ...
    it('calls the onVerbStart and onVerbEnd callbacks if they are defined.', async(): Promise<void> => {
      const dogmEngine = new DOGMEngine({ type: 'memory', callbacks });
      await dogmEngine.save(schemas);
      const response = await dogmEngine.verb.getFile({ account, id: '12345' });
      expect(response).toEqual(expectedGetFileResponse);
      expect(executeOperation).toHaveBeenCalledTimes(1);
    ...
    it('calls the onVerbStart and onVerbEnd callbacks if they are defined in the verb\'s configuration.',
      async(): Promise<void> => {
        const dogmEngine = new DOGMEngine({ type: 'memory' });
        await dogmEngine.save(schemas);
        const response = await dogmEngine.verb.getFile({ account, id: '12345' }, { callbacks });
        expect(response).toEqual(expectedGetFileResponse);
        expect(executeOperation).toHaveBeenCalledTimes(1);
        expect(executeOperation).toHaveBeenCalledWith(
          'FilesGetMetadata',
  ...
  it('throws an error when a valid mapping cannot be found.', async(): Promise<void> => {
    ...
    const dogmEngine = new DOGMEngine({ type: 'memory' });
    await dogmEngine.save(schemas);
    await expect(dogmEngine.verb.getName({ entity: { [RDFS.label]: 'final.jpg' }}))
  ...
  it('throws an error if the operation is not supported.', async(): Promise<void> => {
    ...
    schemas = schemas.map((schemaItem: any): any => {
      if (schemaItem['@id'] === 'https://example.com/data/4') {
        schemaItem['https://dogm.com/ontologies/core/operationMapping']['http://www.w3.org/ns/r2rml#predicateObjectMap'] = [
          {
            '@type': 'http://www.w3.org/ns/r2rml#PredicateObjectMap',
    ...
    const dogmEngine = new DOGMEngine({ type: 'memory' });
    await dogmEngine.save(schemas);
    await expect(dogmEngine.verb.getFile({ account, id: '12345' }))
