...
import { DCTERMS, DOGM, XSD } from '../../../../../src/util/Vocabularies';
import { streamFrom } from '../../../../util/Util';

const endpointUrl = 'https://example.com/sparql';
const file = DataFactory.namedNode(DOGM.File);
...
describe('a SparqlQueryAdapter', (): void => {
    ...
    it('executes a an update query.', async(): Promise<void> => {
      await expect(
        adapter.executeRawUpdate([
          `DELETE { GRAPH <https://example.com/data/1> { <https://example.com/data/1> <${DOGM.sourceId}> ?c1. } }`,
          `INSERT { GRAPH <https://example.com/data/1> { <https://example.com/data/1> <${DOGM.sourceId}> "abc123". } }`,
          'USING <https://example.com/data/1>',
          `WHERE { OPTIONAL { <https://example.com/data/1> <${DOGM.sourceId}> ?c1. } }`,
        ].join('\n')),
    ...
    it('queries for entities and returns an empty array if there are no results.',
      async(): Promise<void> => {
        ...
        expect(select.mock.calls[0][0].split('\n')).toEqual([
          ...
          '  {',
          '    SELECT DISTINCT ?entity WHERE {',
          '      ?entity (<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>/(<http://www.w3.org/2000/01/rdf-schema#subClassOf>*)) <https://dogm.com/ontologies/core/File>.',
          '      FILTER(EXISTS { GRAPH ?entity { ?entity ?c1 ?c2. } })',
          '    }',
          '  }',
          '  GRAPH ?entity { ?subject ?predicate ?object. }',
  ...
  describe('findAllBy', (): void => {
    it('queries for entities and returns an empty array if there are no results.',
      async(): Promise<void> => {
        await expect(
          adapter.findAllBy({ type: DOGM.File }),
        ).resolves.toEqual([]);
        expect(select).toHaveBeenCalledTimes(1);
        expect(select.mock.calls[0][0].split('\n')).toEqual([
          'CONSTRUCT { ?subject ?predicate ?object. }',
  ...
  describe('exists', (): void => {
    it('queries for the existence of an entity matching the where parameter.', async(): Promise<void> => {
      ...
      await expect(adapter.exists({ where: { type: DOGM.File }})).resolves.toBe(true);
    });
  });
  ...
  describe('save', (): void => {
    it('saves a single schema.', async(): Promise<void> => {
      ...
      expect(update.mock.calls[0][0].split('\n')).toEqual([
        ...
        'INSERT DATA { GRAPH <https://example.com/data/1> { <https://example.com/data/1> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <https://dogm.com/ontologies/core/File>. } }',
      ]);
    });
    ...
    it('saves multiple schema.', async(): Promise<void> => {
      const entities = [
        ...
        {
          '@id': 'https://example.com/data/2',
          '@type': 'https://dogm.com/ontologies/core/Article',
        },
      ];
      ...
      expect(update.mock.calls[0][0].split('\n')).toEqual([
        ...
        'INSERT DATA {',
        '  GRAPH <https://example.com/data/1> { <https://example.com/data/1> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <https://dogm.com/ontologies/core/File>. }',
        '  GRAPH <https://example.com/data/2> { <https://example.com/data/2> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <https://dogm.com/ontologies/core/Article>. }',
        '}',
      ]);
    });
  });
  ...
  describe('update', (): void => {
    ...
    it('updates multiple schemas by attribute.', async(): Promise<void> => {
      ...
      expect(update.mock.calls[0][0].split('\n')).toEqual([
        ...
        'USING <https://example.com/data/1>',
        `WHERE { OPTIONAL { <https://example.com/data/1> <${DOGM.sourceId}> ?c1. } };`,
        `DELETE { GRAPH <https://example.com/data/2> { <https://example.com/data/2> <${DOGM.sourceId}> ?c2. } }`,
        `INSERT { GRAPH <https://example.com/data/2> { <https://example.com/data/2> <${DOGM.sourceId}> "abc123". } }`,
        'USING <https://example.com/data/2>',
        `WHERE { OPTIONAL { <https://example.com/data/2> <${DOGM.sourceId}> ?c2. } }`,
      ]);
    });
  });
